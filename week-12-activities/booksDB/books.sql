...
SELECT books.id as books_id, authorId as authors_id, title, firstName, lastName
FROM books
LEFT JOIN authors ON books.authorId = authors.id;

-- show ALL books, even if we don't know the author
-- RIGHT JOIN returns all of the values from the right table, and the matching ones from the left table
SELECT books.id as book_id, authorId, authors.id as author_id, title, firstName, lastName
FROM books
RIGHT JOIN authors ON books.authorId = authors.id;

SELECT books.id as book_id, authorId, authors.id as author_id, title, firstName, lastName
FROM books
OUTER JOIN authors ON books.authorId = authors.id;
